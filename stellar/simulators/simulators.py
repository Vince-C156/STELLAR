...
class simulate_arpod():
    ...
    def __init__(self, model_file: str, inital_conditions: list, deterministic: bool):
        ...
        assert os.path.exists(model_dir), f'{model_file} does not exist in directory {os.path.basename(model_dir)}. Existing options are {os.listdir(model_dir)}'

        #loading environment
        print("Initalizing environment")
        env = make_vec_env(ARPOD_GYM, wrapper_class=TimeLimitWrapper, n_envs=1)
        envpkl = 'vbar-agent_vecnormalize_2250000_steps.pkl'
        envpkl_path = f'../envstats/{envpkl}'
