...
def write2text(chaser, data_dir, file_name, step, opt_id):

    """
    Parameters: 
               opt_id : (0) write position (1) write velocity (2) write control input.
    """
    dir_path = os.getcwd()
    ...
    if opt_id == 0:
        ...
        #print("WRITING CONTROL OUTPUT")
        actuation_arr = np.asarray(chaser.get_u_trace())
        #print(actuation_arr)
        #print(actuation_arr.shape)
        pos_arr = actuation_arr[step-2:, 0:]
    else:
        raise Exception(f'INVALID INPUT OF {opt_id} FOR opt_id IN write2text CALL. VALID OPTIONS INCLUDE 0, 1, 2')
...
class render_visual:
    ...
    def render_animation(self, file_name):
        ...
        #plt.ion()
        #data = os.path.join(self.data_path, file_name)
        ani = animation.FuncAnimation(self.fig, partial(self.animate, file_name=file_name), repeat=False, cache_frame_data=True, interval=10)
        plt.ion()
